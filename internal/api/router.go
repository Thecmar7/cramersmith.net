...
func NewRouter(s *store.Store, a *auth.Auth) http.Handler {
	mux := http.NewServeMux()

	mux.HandleFunc("GET /health", handleHealth)
	mux.HandleFunc("GET /visits", handleVisits(s))
	mux.HandleFunc("GET /posts", handleListPosts(s))
	mux.Handle("POST /posts", a.Middleware(http.HandlerFunc(handleCreatePost(s))))
...
func handleCreatePost(s *store.Store) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var body struct {
